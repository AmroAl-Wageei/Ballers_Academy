...
class ProviderApplicationService
{
    ...
    public function getProviderWithServicesAndSchedule($providerId)
    {
        ...
        $this->setProviderServices($provider, $services, true);
    ...
    private function makePeriodsAvailable($day)
    {
        ...
        foreach ($day->getPeriodList()->getItems() as $key => $period) {
            if ($period->getLocationId() || $period->getPeriodServiceList()->length()) {
                $startInSeconds = $timeSlotService->getSeconds(
                    $period->getStartTime()->getValue()->format('H:i:s')
                );

                $periodsInSeconds[$startInSeconds] = $timeSlotService->getSeconds(
                    $period->getEndTime()->getValue()->format('H:i:s')
                );
            }
        }
    ...
    private function setAvailablePeriodServicesLocations(&$data, $provider, $periodList, $locations)
    {
        ...
        $hasVisibleLocations = $timeSlotService->hasVisibleLocations($locations);
        ...
        foreach ($periodList->getItems() as $period) {
            ...
            $availablePeriodLocations = $timeSlotService->getProviderPeriodLocations(
                $period,
                $providerLocation,
                $locations,
                $hasVisibleLocations
            ...
            if ($period->getPeriodServiceList()->length()) {
                ...
                foreach ($provider->getServiceList()->getItems() as $providerService) {
                    if ($availablePeriodLocations->length()) {
                        /** @var Location $availableLocation */
                        foreach ($availablePeriodLocations->getItems() as $availableLocation) {
                            $data[$providerService->getId()->getValue()][] = $availableLocation->getId()->getValue();
                        }
                    } else {
                        $data[$providerService->getId()->getValue()][] = null;
                    }
                }
            }
        }
