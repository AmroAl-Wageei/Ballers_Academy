    ...
    <template slot="label">
      <span :style="{ 'color': customizationForm.formTextColor }">
        <template v-if="!languageShortCode">
          {{ labelLastName.value || `${$root.labels.last_name_colon}:` }}
        </template>
        <template v-else>
          {{ labelLastName.translations[languageShortCode] || `${$root.labels.last_name_colon}:` }}
        </template>
      </span>
    </template>
  ...
  export default {
    ...
    data () {
      return {
        ...
        labelLastName: this.formField.labels.last_name_colon
      }
    },
    ...
    watch: {
      'formField' () {
        this.labelLastName = this.formField.labels.last_name_colon
      }
    }
  }
</script>
