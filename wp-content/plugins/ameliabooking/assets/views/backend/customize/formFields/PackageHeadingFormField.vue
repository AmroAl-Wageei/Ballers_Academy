<template>
  ...
  >
    <p class="am-service-form-heading" :style="{ 'color': customizationForm.formTextColor }">
      <template v-if="!languageShortCode">
        {{ labelHeading.value || `${$root.labels.please_select_package}:` }}
      </template>
      <template v-else>
        {{ labelHeading.translations[languageShortCode] || `${$root.labels.please_select_package}:` }}
      </template>
    </p>
...
<script>
  ...
  export default {
    ...
    data () {
      return {
        labelHeading: this.formField.labels.please_select_package
      }
    },
    ...
    watch: {
      'formField' () {
        this.labelHeading = this.formField.labels.please_select_package
      }
    }
  }
</script>
