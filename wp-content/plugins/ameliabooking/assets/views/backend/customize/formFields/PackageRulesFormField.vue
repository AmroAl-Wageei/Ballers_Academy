<template>
  ...
  >
    <p class="am-package-appointments-rules" :style="{color: customizationEdit.useGlobalColors ?  customizationForm.textColorOnBackground : customizationForm.formTextColor}">
      <template v-if="!languageShortCode">
        1 {{ labelPackageRules.value || $root.labels.package_min_book }}
      </template>
      <template v-else>
        1 {{ labelPackageRules.translations[languageShortCode] || $root.labels.package_min_book }}
      </template>
    </p>
...
export default {
  ...
  data () {
    return {
      labelPackageRules: this.formField.labels.package_min_book
    }
  },
  ...
  watch: {
    'formField' () {
      this.labelPackageRules = this.formField.labels.package_min_book
    }
  }
}
</script>
