<template>
  ...
  >
    <p class="am-recurring-form-heading" :style="{ 'color': customizationEdit.useGlobalColors ? customizationForm.textColorOnBackground : customizationForm.formTextColor }">
      <template v-if="!languageShortCode">
        {{ recurringHeadingLabel.value || `${$root.labels.recurring_active}:` }}
      </template>
      <template v-else>
        {{ recurringHeadingLabel.translations[languageShortCode] || `${$root.labels.recurring_active}:` }}
      </template>
    </p>
...
export default {
  ...
  data () {
    return {
      recurringHeadingLabel: this.formField.labels.recurring_active
    }
  },
  ...
  watch: {
    'formField' () {
      this.recurringHeadingLabel = this.formField.labels.recurring_active
    }
  }
}
</script>