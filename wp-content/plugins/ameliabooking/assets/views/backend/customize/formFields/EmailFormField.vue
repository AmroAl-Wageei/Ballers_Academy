    ...
    <template slot="label">
      <span :style="{ 'color': customizationForm.formTextColor }">
        <template v-if="!languageShortCode">
          {{ labelEmail.value || `${$root.labels.email_colon}:` }}
        </template>
        <template v-else>
          {{ labelEmail.translations[languageShortCode] || `${$root.labels.email_colon}:` }}
        </template>
      </span>
    </template>
  ...
  export default {
    ...
    data () {
      return {
        ...
        labelEmail: this.formField.labels.email_colon
      }
    },
    ...
    watch: {
      'formField' () {
        this.labelEmail = this.formField.labels.email_colon
      }
    }
  }
</script>
